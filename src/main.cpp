...
// ---- Modes ----
enum HomeMode { MODE_OFF, MODE_AUTO, MODE_ON };
...
float readBatteryVoltage() {
  ...
  float sum = 0;
  for (int i=0; i<BATTERY_SAMPLES; i++) sum += batteryReadings[i];
...
void updateTEC() {
  ...
  bool desired = false;
  switch (homeMode) {
    case MODE_OFF:  desired = false; break;
    case MODE_ON:   desired = true; break;
    case MODE_AUTO:
      if (currentTemp > (setTemp + HYSTERESIS)) desired = true;
      else desired = false;
      break;
  }
...
void updateLCD() {
  ...
  switch (currentScreen) {
    ...
    case SCREEN_MENU: {
      strcpy(newLCD[0], "MENU"); newLen[0] = 4;
      newLCD[1][0] = '\0'; newLen[1] = 0;
      const char* items[] = {"RUN", "SET"};
      for (int i=0; i<2; i++) {
        char line[LCD_COLS+1];
        if (i == menuIndex) snprintf(line, LCD_COLS+1, ">%s", items[i]);
        else snprintf(line, LCD_COLS+1, " %s", items[i]);
    ...
    case SCREEN_SET_TEMP: {
      strcpy(newLCD[0], "SET TEMP"); newLen[0] = 8;
      newLCD[1][0] = '\0'; newLen[1] = 0;
      ...
      strcpy(newLCD[2], setLine); newLen[2] = strlen(setLine);
      char curBuf[LCD_COLS+1];
      if (currentTemp > -100)
        snprintf(curBuf, LCD_COLS+1, "Cur: %4.1f C", currentTemp);
      else
        snprintf(curBuf, LCD_COLS+1, "Cur: ERR");
      strcpy(newLCD[3], curBuf); newLen[3] = strlen(curBuf);
      break;
    }

    case SCREEN_RUN: {
      strcpy(newLCD[0], "RUNNING"); newLen[0] = 7;
      char buf[LCD_COLS+1];
      snprintf(buf, LCD_COLS+1, "Set:%4.1fC Cur:%4.1fC", setTemp, currentTemp);
      strcpy(newLCD[1], buf); newLen[1] = strlen(buf);
      const char* tecStr = !tecAllowed ? "OFF-LOW" : (tecState ? "ON " : "OFF");
      snprintf(buf, LCD_COLS+1, "TEC:%s Bat:%5.2fV", tecStr, batteryVoltage);
      strcpy(newLCD[2], buf); newLen[2] = strlen(buf);
      strcpy(newLCD[3], "STOP"); newLen[3] = 4;
      break;
    }

    case SCREEN_TEST_TEC: {
      strcpy(newLCD[0], "TEST TEC"); newLen[0] = 8;
      newLCD[1][0] = '\0'; newLen[1] = 0;
      newLCD[2][0] = '\0'; newLen[2] = 0;
      char buf[LCD_COLS+1];
      if (blinkState) {
      ...
      strcpy(newLCD[3], buf); newLen[3] = strlen(buf);
      break;
    }
  }
  ...
  for (int r=0; r<LCD_ROWS; r++) {
    ...
    lastLen[r] = newLen[r];
    if (newLen[r] < LCD_COLS) lastLCD[r][newLen[r]] = '\0';
  }
}
...
void setup() {
  ...
  for (int i=0; i<BATTERY_SAMPLES; i++) batteryReadings[i] = batteryVoltage;
  batteryIndex = 0;

  sensors.requestTemperatures();
  delay(100);
  currentTemp = sensors.getTempCByIndex(0);
  if (currentTemp == DEVICE_DISCONNECTED_C) currentTemp = -999.0;
  tempRequestPending = false;
  ...
  lcd.clear();
  for (int r=0; r<LCD_ROWS; r++) { lastLCD[r][0]='\0'; lastLen[r]=0; }
  updateLCD();
